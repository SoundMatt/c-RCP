//cfusa:test REQ-SAFEPT-001
//cfusa:test REQ-SAFEPT-002
//cfusa:test REQ-SAFEPT-003
//cfusa:test REQ-SAFEPT-004
//cfusa:test REQ-SAFEPT-005
//cfusa:test REQ-SAFEPT-006
//cfusa:test REQ-SAFEPT-007
//cfusa:test REQ-SAFEPT-008
//cfusa:test REQ-SAFEPT-009
//cfusa:test REQ-SAFEPT-010
//cfusa:test REQ-SAFEPT-011
//cfusa:test REQ-SAFEPT-012
//cfusa:test REQ-SAFEPT-013
//cfusa:test REQ-SAFEPT-014
//cfusa:test REQ-SAFEPT-015
//cfusa:test REQ-SAFEPT-016
//cfusa:test REQ-SAFEPT-017
//cfusa:test REQ-SAFEPT-018
//cfusa:test REQ-SAFEPT-019
//cfusa:test REQ-SAFEPT-020
//cfusa:test REQ-SAFEPT-021
//cfusa:test REQ-SAFEPT-022
//cfusa:test REQ-SAFEPT-023
//cfusa:test REQ-SAFEPT-024
//cfusa:test REQ-SAFEPT-025
//cfusa:test REQ-SAFEPT-026
//cfusa:test REQ-SAFEPT-027
#include "unity.h"
...
{
    const char *a   = rcp_safept_strerror(RCP_SAFEPT_OK);
    const char *b   = rcp_safept_strerror(RCP_SAFEPT_ERR_SHORT_FRAME);
    const char *c   = rcp_safept_strerror(RCP_SAFEPT_ERR_CRC_MISMATCH);
    const char *unk = rcp_safept_strerror((rcp_safept_errc_t)999);
...
static void test_crc32_known_answer_vector(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX32(0x1697D06Au, rcp_safept_crc32(vec, 9));
}

static void test_crc32_empty_input(void)
{
    /* CRC of nothing is init XORed with final XOR, i.e. 0. */
    TEST_ASSERT_EQUAL_HEX32(0u, rcp_safept_crc32(NULL, 0));
}
...
static void test_crc32_differs_for_different_data(void)
{
    ...
    TEST_ASSERT_NOT_EQUAL(rcp_safept_crc32(a, sizeof(a)), rcp_safept_crc32(b, sizeof(b)));
}
...
static void test_compute_crc_matches_manual_concatenation(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX32(rcp_safept_crc32(concat, sizeof(concat)),
                             rcp_safept_compute_crc(stream_id, avtp_timestamp, acf_frame,
                                                     sizeof(acf_frame)));
}
...
static void test_compute_crc_zero_stream_and_timestamp_ntscf_standin(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX32(rcp_safept_crc32(concat, sizeof(concat)),
                             rcp_safept_compute_crc(0, 0, acf_frame, 3));
}
...
{
    TEST_ASSERT_EQUAL_UINT((size_t)0 + RCP_SAFEPT_CRC_LEN, rcp_safept_length_with_crc(0));
    TEST_ASSERT_EQUAL_UINT((size_t)100 + RCP_SAFEPT_CRC_LEN, rcp_safept_length_with_crc(100));
}

static void test_length_with_crc_saturates(void)
{
    TEST_ASSERT_EQUAL_UINT((size_t)-1, rcp_safept_length_with_crc((size_t)-1));
    TEST_ASSERT_EQUAL_UINT((size_t)-1, rcp_safept_length_with_crc((size_t)-1 - 1));
}
...
static void test_wrap_appends_crc_len_bytes(void)
{
    ...
    out = rcp_safept_wrap(42, 7, acf_frame, sizeof(acf_frame));
    TEST_ASSERT_NOT_NULL(out.data);
    TEST_ASSERT_EQUAL_UINT(sizeof(acf_frame) + RCP_SAFEPT_CRC_LEN, out.len);
...
{
    rcp_bytes_t out = rcp_safept_wrap(1, 1, NULL, 4);
...
static void test_wrap_unwrap_round_trip_ok(void)
{
    ...
    size_t body_len = 0;
    rcp_safept_errc_t rc;

    wrapped = rcp_safept_wrap(0xDEADBEEFu, 0xCAFEu, acf_frame, sizeof(acf_frame));
    TEST_ASSERT_NOT_NULL(wrapped.data);

    rc = rcp_safept_unwrap(0xDEADBEEFu, 0xCAFEu, wrapped.data, wrapped.len, &body, &body_len);
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_OK, rc);
...
static void test_unwrap_short_frame(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_ERR_SHORT_FRAME,
                           rcp_safept_unwrap(0, 0, buf, sizeof(buf), &body, &body_len));
}
...
static void test_unwrap_crc_mismatch_on_corruption(void)
{
    ...
    wrapped = rcp_safept_wrap(5, 5, acf_frame, sizeof(acf_frame));
    TEST_ASSERT_NOT_NULL(wrapped.data);

    wrapped.data[0] ^= 0xFFu; /* corrupt the payload, not the trailer */

    rc = rcp_safept_unwrap(5, 5, wrapped.data, wrapped.len, &body, &body_len);
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_ERR_CRC_MISMATCH, rc);
...
static void test_unwrap_crc_mismatch_on_wrong_stream_id(void)
{
    ...
    wrapped = rcp_safept_wrap(1, 1, acf_frame, sizeof(acf_frame));
    TEST_ASSERT_NOT_NULL(wrapped.data);

    /* Same bytes, but unwrapped against the wrong stream_id -- the
     * coverage span includes stream_id, so this must fail too. */
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_ERR_CRC_MISMATCH,
                           rcp_safept_unwrap(2, 1, wrapped.data, wrapped.len, &body, &body_len));
...
{
    TEST_ASSERT_TRUE(rcp_safept_fragment_carries_crc(true));
    TEST_ASSERT_FALSE(rcp_safept_fragment_carries_crc(false));
}
...
{
    TEST_ASSERT_TRUE(rcp_safept_is_safety_request(0x8Fu));  /* compound, safety */
    TEST_ASSERT_TRUE(rcp_safept_is_safety_request(0x8Bu));  /* compound-wait, safety */
    TEST_ASSERT_TRUE(rcp_safept_is_safety_request(0x8Eu));  /* triggered, safety */
    TEST_ASSERT_FALSE(rcp_safept_is_safety_request(0x0Fu)); /* compound */
    TEST_ASSERT_FALSE(rcp_safept_is_safety_request(0x0Bu)); /* compound-wait */
    TEST_ASSERT_FALSE(rcp_safept_is_safety_request(0x0Eu)); /* triggered */
    TEST_ASSERT_FALSE(rcp_safept_is_safety_request(0x00u)); /* standard */
}

static void test_request_may_execute_safety_gated_on_safe_state(void)
{
    TEST_ASSERT_TRUE(rcp_safept_request_may_execute(0x8Fu, true));
    TEST_ASSERT_FALSE(rcp_safept_request_may_execute(0x8Fu, false));
}

static void test_request_may_execute_non_safety_always_permitted(void)
{
    TEST_ASSERT_TRUE(rcp_safept_request_may_execute(0x0Fu, true));
    TEST_ASSERT_TRUE(rcp_safept_request_may_execute(0x0Fu, false));
}
...
{
    TEST_ASSERT_TRUE(rcp_safept_watchdog_purge_should_keep(0x8Fu));
    TEST_ASSERT_TRUE(rcp_safept_watchdog_purge_should_keep(0x8Bu));
    TEST_ASSERT_TRUE(rcp_safept_watchdog_purge_should_keep(0x8Eu));
    TEST_ASSERT_FALSE(rcp_safept_watchdog_purge_should_keep(0x0Fu));
    TEST_ASSERT_FALSE(rcp_safept_watchdog_purge_should_keep(0x0Bu));
    TEST_ASSERT_FALSE(rcp_safept_watchdog_purge_should_keep(0x0Eu));
    TEST_ASSERT_FALSE(rcp_safept_watchdog_purge_should_keep(0x00u));
}
...
static void test_watchdog_purge_classify_matches_per_entry_rule(void)
{
    ...
    rcp_safept_watchdog_purge_classify(types, 6, keep);
...
{
    rcp_safept_watchdog_purge_classify(NULL, 0, NULL);
...
{
    TEST_ASSERT_TRUE(rcp_safept_measure_valid((uint8_t)RCP_SAFEPT_MEASURE_FORCE_HIGH_IMPEDANCE));
    TEST_ASSERT_TRUE(rcp_safept_measure_valid((uint8_t)RCP_SAFEPT_MEASURE_SEQUENCER));
    TEST_ASSERT_FALSE(rcp_safept_measure_valid(2));
    TEST_ASSERT_FALSE(rcp_safept_measure_valid(0xFFu));
}

static void test_endpoint_in_safe_state_force_high_impedance_always_true(void)
{
    TEST_ASSERT_TRUE(rcp_safept_endpoint_in_safe_state(
        (uint8_t)RCP_SAFEPT_MEASURE_FORCE_HIGH_IMPEDANCE, NULL, 0, 0));
}
...
static void test_endpoint_in_safe_state_sequencer_matches_target_state(void)
{
    ...
    TEST_ASSERT_FALSE(rcp_safept_endpoint_in_safe_state(
        (uint8_t)RCP_SAFEPT_MEASURE_SEQUENCER, &table, 2, 5));
...
{
    TEST_ASSERT_FALSE(rcp_safept_endpoint_in_safe_state(0xFFu, NULL, 0, 0));
}

static void test_endpoint_in_safe_state_fails_closed_on_null_table(void)
{
    TEST_ASSERT_FALSE(rcp_safept_endpoint_in_safe_state(
        (uint8_t)RCP_SAFEPT_MEASURE_SEQUENCER, NULL, 0, 1));
}
...
static void test_endpoint_in_safe_state_fails_closed_on_invalid_index(void)
{
    ...
    TEST_ASSERT_FALSE(rcp_safept_endpoint_in_safe_state(
        (uint8_t)RCP_SAFEPT_MEASURE_SEQUENCER, &table, 99, 1));
...
{
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_CRC_ACTION_DROP_REQUEST, rcp_safept_crc_error_action(false));
    TEST_ASSERT_EQUAL_INT(RCP_SAFEPT_CRC_ACTION_LATCH_STREAM_FAULT, rcp_safept_crc_error_action(true));
}

static void test_stream_fault_drop_request_never_latches(void)
{
    rcp_safept_stream_fault_t f;
    rcp_safept_stream_fault_init(&f);

    TEST_ASSERT_TRUE(rcp_safept_stream_fault_on_crc_error(&f, false));
    TEST_ASSERT_FALSE(rcp_safept_stream_fault_is_faulted(&f));
...
static void test_stream_fault_latches_and_stays_latched_until_reset(void)
{
    ...
    TEST_ASSERT_TRUE(rcp_safept_stream_fault_on_crc_error(&f, true));
    TEST_ASSERT_TRUE(rcp_safept_stream_fault_is_faulted(&f));
    ...
    rcp_safept_stream_fault_reset(&f);
    TEST_ASSERT_FALSE(rcp_safept_stream_fault_is_faulted(&f));
}
...
{
    rcp_safept_wd_result_t r = rcp_safept_wd_evaluate(false, 10, true, true, 1000000);
...
{
    rcp_safept_wd_result_t r = rcp_safept_wd_evaluate(true, 100, true, true, 99);
    TEST_ASSERT_FALSE(r.overflowed);
    TEST_ASSERT_FALSE(r.enter_safe_state);
    TEST_ASSERT_FALSE(r.notify);
}
...
{
    rcp_safept_wd_result_t r1 = rcp_safept_wd_evaluate(true, 100, true, true, 100);
    rcp_safept_wd_result_t r2 = rcp_safept_wd_evaluate(true, 100, true, true, 500);
...
{
    rcp_safept_wd_result_t both = rcp_safept_wd_evaluate(true, 10, true, true, 50);
    rcp_safept_wd_result_t only_safestate = rcp_safept_wd_evaluate(true, 10, true, false, 50);
    rcp_safept_wd_result_t only_notify = rcp_safept_wd_evaluate(true, 10, false, true, 50);
    rcp_safept_wd_result_t neither = rcp_safept_wd_evaluate(true, 10, false, false, 50);
...
static void test_watchdog_overflow_drives_the_full_purge_and_survive_flow(void)
{
    ...
    bool keep[4] = {false};
    rcp_safept_wd_result_t wd;
    rcp_sequencer_table_t table = rcp_sequencer_table_new(1);

    wd = rcp_safept_wd_evaluate(true, 50, true, false, 200);
    TEST_ASSERT_TRUE(wd.overflowed);
    TEST_ASSERT_TRUE(wd.enter_safe_state);

    rcp_safept_watchdog_purge_classify(queued, 4, keep);
    TEST_ASSERT_FALSE(keep[0]);
    TEST_ASSERT_TRUE(keep[1]);
    TEST_ASSERT_FALSE(keep[2]);
    TEST_ASSERT_TRUE(keep[3]);
    ...
    TEST_ASSERT_FALSE(rcp_safept_request_may_execute(0x8Fu, false));
    TEST_ASSERT_FALSE(rcp_safept_request_may_execute(0x8Eu, false));

    /* ...once the sequencer-based safe state is actually reached, they
     * may. */
    TEST_ASSERT_TRUE(rcp_sequencer_set_state(&table, 0, 9));
    TEST_ASSERT_TRUE(rcp_safept_endpoint_in_safe_state(
        (uint8_t)RCP_SAFEPT_MEASURE_SEQUENCER, &table, 0, 9));
    TEST_ASSERT_TRUE(rcp_safept_request_may_execute(0x8Fu, true));
    TEST_ASSERT_TRUE(rcp_safept_request_may_execute(0x8Eu, true));
