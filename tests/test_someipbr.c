 ...
 *
 * rcp_someip_bridge_send() is a compile-time interface stub: until a
 * concrete SOME/IP backend is linked, it always returns
 * RCP_ERR_NOT_SUPPORTED and never touches *out_response. These tests pin
...
static void test_send_returns_not_supported_when_stub(void)
{
    ...
    TEST_ASSERT_EQUAL(RCP_ERR_NOT_SUPPORTED,
                       rcp_someip_bridge_send(rcp_someip_default_config(), make_addr(), 0x00,
...
{
    rcp_someip_config_t cfg = rcp_someip_default_config();

    TEST_ASSERT_EQUAL_HEX16(0x0100, cfg.service_id);
    TEST_ASSERT_EQUAL_HEX16(0x0001, cfg.instance_id);
    TEST_ASSERT_EQUAL_HEX16(0x0001, cfg.method_id);
