 ...
 *
 * The SOME/IP controller is a compile-time interface stub: until a
 * concrete backend is linked, send()/subscribe() return
...
//cfusa:test REQ-SOMEIP-003
//cfusa:test REQ-SOMEIP-004
#include "unity.h"
...
{
    rcp_controller_t *ctrl = rcp_someip_controller_new(RCP_ZONE_FRONT_LEFT, rcp_someip_default_config());
...
{
    rcp_controller_t *ctrl = rcp_someip_controller_new(RCP_ZONE_REAR_RIGHT, rcp_someip_default_config());
...
{
    rcp_controller_t *ctrl = rcp_someip_controller_new(RCP_ZONE_CENTRAL, rcp_someip_default_config());
    rcp_context_t ctx = rcp_context_background();
...
{
    rcp_controller_t *ctrl = rcp_someip_controller_new(RCP_ZONE_FRONT_RIGHT, rcp_someip_default_config());
