 ...
 * another kind.
 *
 * Two ranges of the REQ-MOCK id space (c-RCP-18 requirement-atomicity
 * audit, issue #533, Group 4 residual) used to be listed as two separate
 * flat blocks in this file's own header, alongside every other prefix's
 * own id this file also covers (REQ-SRV, REQ-ACF, REQ-CANCEL, REQ-TIMED).
 * Each REQ-MOCK id now carries its own individual marker placed directly
 * at the specific test function(s) its own text actually describes; every
 * other prefix's own lines above are untouched, left for their own future
 * dedicated pass. One REQ-MOCK id's own strongest evidence turned out to
 * live in tests/test_mock.c instead (its own fixture registers two
 * endpoints on two streams; this file's own fixture() registers only one,
 * so no test here can demonstrate that id's cross-endpoint fan-out) --
 * see that file's own marker comment for the reasoning.
 */
#include "unity.h"
...
/* Also REQ-MOCK-024's own clearest demonstration: the condition below is
 * driven ENTIRELY through rcp_mock_server_sequencers(srv) -- the server's
 * own table -- while every tick() call here goes through base_ctx(),
 * whose own ctx.sequencers stays NULL throughout (see that helper's own
 * comment). tick() still evaluates the real condition correctly, which is
 * only possible if it substitutes its own table for whatever (here,
 * nothing) ctx carried in. */
