/* See legacy_mock.h for what this file is and why it exists. */
#include "legacy_mock.h"

#include <stdlib.h>
#include <string.h>

/* src/platform.h is an internal, not-installed library header (portable
 * mutex/thread primitives) -- this test-only translation unit reaches
 * across to it directly rather than duplicating those primitives, the same
 * way the original src/mock.c did before this file's relocation (see
 * legacy_mock.h). */
