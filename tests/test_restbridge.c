 ...
 *
 * rcp_rest_bridge_send() is a compile-time interface stub: until a concrete
 * REST backend is linked, it always returns RCP_ERR_NOT_SUPPORTED and never
...
static void test_send_returns_not_supported_when_stub(void)
{
    ...
    TEST_ASSERT_EQUAL(RCP_ERR_NOT_SUPPORTED,
                       rcp_rest_bridge_send(rcp_rest_default_config(), make_addr(), 0x00,
...
{
    rcp_rest_config_t cfg = rcp_rest_default_config();

    TEST_ASSERT_NULL(cfg.base_url);
    TEST_ASSERT_EQUAL(3, cfg.max_retries);
    TEST_ASSERT_EQUAL_UINT64(1000, cfg.request_timeout_ms);
}
