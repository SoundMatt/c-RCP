 ...
 * UDS backend is linked, it always returns RCP_ERR_NOT_SUPPORTED and never
 ...
 * rcp_uds_default_config()'s documented defaults.
 */
#include "unity.h"
...
static void test_send_returns_not_supported_when_stub(void)
{
    ...
    TEST_ASSERT_EQUAL(RCP_ERR_NOT_SUPPORTED,
                       rcp_uds_bridge_send(rcp_uds_default_config(), make_addr(), 0x00,
...
{
    rcp_uds_config_t cfg = rcp_uds_default_config();

    TEST_ASSERT_EQUAL_HEX16(0x0100, cfg.routine_id);
    TEST_ASSERT_EQUAL_UINT64(50, cfg.p2_timeout_ms);
    TEST_ASSERT_EQUAL_UINT64(5000, cfg.p2ext_timeout_ms);
}
