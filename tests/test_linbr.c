 ...
 * concrete LIN backend is linked, it always returns RCP_ERR_NOT_SUPPORTED
 ...
 * separately pin rcp_lin_default_config()'s documented defaults. See
...
static void test_send_returns_not_supported_when_stub(void)
{
    ...
    TEST_ASSERT_EQUAL(RCP_ERR_NOT_SUPPORTED,
                       rcp_lin_bridge_send(rcp_lin_default_config(), make_addr(), 0x00,
...
{
    rcp_lin_config_t cfg = rcp_lin_default_config();

    TEST_ASSERT_EQUAL_HEX8(0x10, cfg.frame_id);
    TEST_ASSERT_EQUAL_UINT64(50, cfg.timeout_ms);
}
