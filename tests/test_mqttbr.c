 ...
 *
 * rcp_mqtt_bridge_send() is a compile-time interface stub: until a concrete
 * MQTT backend is linked, it always returns RCP_ERR_NOT_SUPPORTED and never
...
static void test_send_returns_not_supported_when_stub(void)
{
    ...
    TEST_ASSERT_EQUAL(RCP_ERR_NOT_SUPPORTED,
                       rcp_mqtt_bridge_send(rcp_mqtt_default_config(), make_addr(), 0x00,
...
{
    rcp_mqtt_config_t cfg = rcp_mqtt_default_config();

    TEST_ASSERT_EQUAL_STRING("tcp://localhost:1883", cfg.broker_url);
    TEST_ASSERT_EQUAL_STRING("rcp", cfg.topic_prefix);
    TEST_ASSERT_EQUAL(1, cfg.qos);
    TEST_ASSERT_EQUAL_UINT64(1000, cfg.timeout_ms);
}
