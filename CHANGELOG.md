...
### v0.450.0 -- 2026-08-18 ([c-RCP-18-tracker] issue #533 batch REQ-I2C-*: requirement-atomicity audit, Group 2 per-endpoint)
...
combined-flag reporting-bug finding) each 100% (1233/1233 requirements,
512/512 functions).

Version 0.449.0 -> 0.450.0 (`CMakeLists.txt`/`version.h`/`.fusa.json`
kept in sync, renumbered past the concurrently-merged `REQ-SPI-*`
(`v0.444.0`), `REQ-ADC-*` (`v0.445.0`), `REQ-LINEP-*` (`v0.446.0`),
`REQ-GPIO-*` (`v0.447.0`), `REQ-ISELED-*` (`v0.448.0`), and
`REQ-CANEP-*` (`v0.449.0`) batches); dated `CHANGELOG.md`/`ROADMAP.md`
entries above the current highest version header in each.

Part of #533. Not closing it -- other Group 2 (`REQ-MDIO-*`,
`REQ-PWM-*`, `REQ-UART-*`, `REQ-WAKEUP-*` -- `REQ-SPI-*`/`REQ-ADC-*`/
`REQ-LINEP-*`/`REQ-GPIO-*`/`REQ-ISELED-*`/`REQ-CANEP-*` landed
