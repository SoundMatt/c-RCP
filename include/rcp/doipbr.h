 ...
 * an external DoIP endpoint is exactly as useful a job under TC18 as it was
 * before -- this module's config shape (rcp_doip_config_t) and its
 * compile-time-stub nature are unchanged. What changes is only the call
 * shape it's framed around: the old rcp_doip_controller_new() wrapped a
 ...
 * rcp_doip_bridge_send() is now the whole entry point, called directly by
 ...
 * that opaque byte string to/from the external DoIP endpoint cfg describes,
 * never decoding it.
 *
 * This is still a compile-time interface stub: no DoIP stack integration is
 * linked, so rcp_doip_bridge_send() always returns RCP_ERR_NOT_SUPPORTED
 * rather than encapsulating a UDS request inside a DoIP diagnostic message
 * over TCP/IP, leaving *out_response untouched -- mirrors cpp-RCP's own
 * doipbr.hpp, which ships the same stub absent a linked DoIP backend. Wiring
 * in a real DoIP backend is future work (see ROADMAP.md).
 *
 ...
 * for the endpoint at addr to a DoIP diagnostic message described by cfg.
 ...
 * always returns RCP_ERR_NOT_SUPPORTED (no DoIP backend linked).
 */
