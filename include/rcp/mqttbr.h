/*
 * mqttbr.h -- MQTT protocol bridge interface stub (SG-006) for the TC18
 ...
 * an external MQTT endpoint is exactly as useful a job under TC18 as it was
 * before -- this module's config shape (rcp_mqtt_config_t) and its
 * compile-time-stub nature are unchanged. What changes is only the call
 * shape it's framed around: the old rcp_mqtt_controller_new() wrapped a
 ...
 * rcp_mqtt_bridge_send() is now the whole entry point, called directly by
 ...
 * that opaque byte string to/from the external MQTT endpoint cfg describes,
 * never decoding it.
 *
 * This is still a compile-time interface stub: no MQTT client library (e.g.
 * Eclipse Paho) is linked, so rcp_mqtt_bridge_send() always returns
 * RCP_ERR_NOT_SUPPORTED rather than publishing to an unconfigured broker,
 * leaving *out_response untouched -- mirrors cpp-RCP's own mqttbr.hpp, which
 * ships the same stub absent a linked MQTT backend. Wiring in a real MQTT
 ...
 * for the endpoint at addr to MQTT topics described by cfg. request_type is
 * passed through opaque, unexamined, purely so a real backend could one day
 * route/label the call. On success, populates *out_response with a freshly
 * heap-allocated response the caller frees with rcp_bytes_free(); on
 * failure, *out_response is left untouched. Currently always returns
 * RCP_ERR_NOT_SUPPORTED (no MQTT backend linked).
 */
