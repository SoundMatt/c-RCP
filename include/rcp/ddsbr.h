 ...
 * 21, "Satellite Package Rework", milestone 81, "Protocol bridges").
 *
 * ADAPT-class rebind, not a from-scratch REPLACE: framing an RCP request for
 * an external DDS endpoint is exactly as useful a job under TC18 as it was
 * before -- this module's config shape (rcp_dds_config_t) and its
 * compile-time-stub nature are unchanged. What changes is only the call
 * shape it's framed around: the old rcp_dds_controller_new() wrapped a whole
 ...
 * rcp_dds_bridge_send() is now the whole entry point, called directly by the
 ...
 * that opaque byte string to/from the external DDS endpoint cfg describes,
 * never decoding it.
 *
 * This is still a compile-time interface stub: no OMG DDS implementation
 * (e.g. FastDDS, Cyclone DDS) is linked, so rcp_dds_bridge_send() always
 * returns RCP_ERR_NOT_SUPPORTED rather than publishing to an unconfigured
 * topic, leaving *out_response untouched -- mirrors cpp-RCP's own ddsbr.hpp,
 * which ships the same stub absent a linked DDS backend. Wiring in a real
 * DDS backend is future work (see ROADMAP.md).
 *
...
typedef struct {
    const char *topic_prefix;    /* DDS topic names: {prefix}/request,
 ...
 * for the endpoint at addr to DDS typed topics described by cfg.
 ...
 * always returns RCP_ERR_NOT_SUPPORTED (no DDS backend linked).
 */
