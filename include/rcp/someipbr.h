/*
 * someipbr.h -- SOME/IP protocol bridge interface stub (SG-006) for the TC18
 ...
 * an external SOME/IP endpoint is exactly as useful a job under TC18 as it
 * was before -- this module's config shape (rcp_someip_config_t) and its
 * compile-time-stub nature are unchanged. What changes is only the call
 * shape it's framed around: the old rcp_someip_controller_new() wrapped a
 ...
 * rcp_someip_bridge_send() is now the whole entry point, called directly by
 ...
 * that opaque byte string to/from the external SOME/IP endpoint cfg
 * describes, never decoding it.
 *
 * This is still a compile-time interface stub: no vsomeip (or equivalent)
 * backend is linked, so rcp_someip_bridge_send() always returns
 * RCP_ERR_NOT_SUPPORTED rather than attempting a service call over an
 * unconfigured channel, leaving *out_response untouched -- mirrors cpp-RCP's
 * own someipbr.hpp, which ships the same stub absent a linked SOME/IP
 * backend. Wiring in a real SOME/IP backend is future work (see ROADMAP.md).
 *
 ...
 * for the endpoint at addr to a SOME/IP service method described by cfg.
 ...
 * always returns RCP_ERR_NOT_SUPPORTED (no SOME/IP backend linked).
 */
