...
| Safety Requirements | `.fusa-reqs.json` | 1137 requirements; 1105 `scope: "tc18"` (1001 ASIL-B / 34 ASIL-A / 70 QM — this project's ISO 26262 safety-case basis), 19 `scope: "tc18-gap"` (normative-clause coverage markers, QM by definition), 7 `scope: "retired"`, 6 `scope: "internal"` — see the file's own `catalogNote` and `FREEDOM_FROM_INTERFERENCE.md` §1 for the current, verified breakdown (supersedes this table's own earlier 854/779/"legacy-compat" and 947/136 figures — the counts moved again via the [c-RCP-16 follow-up] issue #552 fix (`REQ-ISELED-028`: `tc18-gap` → `retired`) and the [c-RCP-18-tracker] issue #533 Group 3 requirement-atomicity splits, both after the issue #548 pass that reclassified 116 `tc18-gap` entries back to `scope: "tc18"` with a real ASIL rating) |
...
(1137 requirements at HEAD: 1105 `scope: "tc18"` covering the
